...
const EmployerManagement = () => {
  ...
  const handleAssignCompany = async () => {
    ...
    try {
      ...
      const response = await httpClient.patch(
        API_ENDPOINTS.ASSIGN_COMPANY_TO_EMPLOYER(
          searchedUser.userId,
  ...
  const handleElevateToEmployer = async (userId) => {
    ...
    try {
      const response = await httpClient.patch(
        API_ENDPOINTS.ELEVATE_TO_EMPLOYER(userId)
      );
      setSuccess(
        `Successfully elevated ${response.data.name} to ROLE_EMPLOYER`
      );
      setSearchedUser(response.data);
    } catch (err) {
      console.error("Error elevating user:", err);
      setError(
        err.response?.data?.message || "Failed to elevate user to employer"
      );
    }
  };
